...
//     enabled: true
//
// The public frontend key is supplied at runtime. It may rotate on a VDAB
// redeploy; when configured, a 403 still gets one self-heal attempt
// (deriveKeyFromBundle) that reads the current key from VDAB's live bundle.
//
...
const VEJ_KEY_ENV = 'VDAB_VEJ_KEY_MONITOR';
...
function configuredVejKey() {
  const value = typeof process !== 'undefined' && process.env
    ? String(process.env[VEJ_KEY_ENV] || '').trim()
    : '';
  if (!value) {
    throw new Error(`vdab: missing ${VEJ_KEY_ENV}; set it before scraping`);
  }
  return value;
}
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    // Scoped to this fetch() call: use the configured key first (fast path);
    ...
    let activeKey = configuredVejKey();
