...
fn macos_update_block_reason(
    ...
    if app_path.contains("/AppTranslocation/") {
        return Some(
            "GalaxyHire is running from macOS Gatekeeper App Translocation. Move GalaxyHire.app to /Applications or ~/Applications, open it from there, then run the update again.".into(),
        );
    }

    if app_path.starts_with("/Volumes/") || install_path.starts_with("/Volumes/") {
        return Some(
            "GalaxyHire is running from a mounted disk image, which macOS exposes as read-only. Drag GalaxyHire.app into /Applications or ~/Applications, open the installed copy, then run the update again.".into(),
        );
    }

    if let Err(error) = writable_result {
        return Some(format!(
            "GalaxyHire cannot write to its install folder ({install_path}). Install the latest DMG manually into a writable Applications folder, then future in-app updates can continue. Details: {error}"
        ));
    }
...
mod tests {
    ...
    fn finds_macos_app_bundle_from_executable_path() {
        let exe = Path::new("/Applications/GalaxyHire.app/Contents/MacOS/GalaxyHire");
        let bundle = find_app_bundle_path(exe).expect("bundle path");

        assert_eq!(bundle, Path::new("/Applications/GalaxyHire.app"));
    }

    #[test]
    fn blocks_updates_from_mounted_disk_image() {
        let reason = macos_update_block_reason(
            Path::new("/Volumes/GalaxyHire/GalaxyHire.app"),
            Path::new("/Volumes/GalaxyHire"),
            Ok(()),
        )
    ...
    fn blocks_updates_from_app_translocation() {
        let reason = macos_update_block_reason(
            Path::new("/private/var/folders/xx/AppTranslocation/123/d/GalaxyHire.app"),
    ...
    fn blocks_updates_when_install_folder_is_not_writable() {
        let reason = macos_update_block_reason(
            Path::new("/Applications/GalaxyHire.app"),
    ...
    fn allows_updates_from_writable_applications_folder() {
        let reason = macos_update_block_reason(
            Path::new("/Users/alice/Applications/GalaxyHire.app"),
    ...
    #[cfg(target_os = "macos")]
    {
        let exe = match std::env::current_exe() {
            ...
            Err(error) => {
                return UpdateInstallStatus {
                    ...
                    reason: format!(
                        "GalaxyHire could not determine its app location before updating: {error}"
                    ),
                    install_dir: None,
                    app_bundle: None,
                };
        ...
        let app_bundle = match find_app_bundle_path(&exe) {
            ...
            None => {
                return UpdateInstallStatus {
                    ...
                    reason: "GalaxyHire is not running from a macOS app bundle.".into(),
        ...
        let install_dir = match app_bundle.parent() {
            ...
            None => {
                return UpdateInstallStatus {
                    ...
                    needs_manual_install: true,
                    reason:
                        "GalaxyHire could not determine the folder that contains the app bundle."
                            .into(),
                    install_dir: None,
        ...
        return UpdateInstallStatus {
            ...
            reason: reason.unwrap_or_else(|| {
                "GalaxyHire is installed in a writable location and can use in-app updates.".into()
            }),
...
fn packaged_sidecar_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    let checked = candidates
        ...
        .join("; ");
    Err(format!(
        "Bundled backend sidecar was not found. Checked: {checked}. Reinstall GalaxyHire or rebuild the package with `npm run build:sidecar` before `tauri build`."
    ))
}
...
fn cleanup_stale_sidecar(app: &AppHandle) {
    ...
    if is_jhm_process(pid) {
        ...
        eprintln!("[tauri] Ignoring stale sidecar pid file for non-GalaxyHire process: {pid}");
    }
    let _ = std::fs::remove_file(pid_path);
}
