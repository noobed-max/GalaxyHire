...
describe("lead utility formatting helpers", () => {
  ...
  it("matches location components, Bengaluru aliases, and remote roles", () => {
    expect(locationMatches("Bengaluru, Karnataka, India", "Bangalore, India")).toBe(true);
    expect(locationMatches("Bangalore, Karnataka, India", "Bengaluru")).toBe(true);
    expect(locationMatches("Remote", "India")).toBe(true);
    expect(locationMatches("Pune, India", "Bengaluru, India")).toBe(false);
  });
