...
/** Manual fallback for desktop update repair. Only rendered under Tauri. */
const RELEASES_URL = "https://github.com/noobed-max/GalaxyHire/releases/latest";
...
export function UpdatePrompt() {
  ...
  const updateMessage = (() => {
    if (state === "ready") return "The update is installed. Restart to finish if GalaxyHire did not reopen automatically.";
    if (state === "relaunching") return "Update installed. Reopening GalaxyHire with the new version.";
    if (blockedByInstallLocation) return installStatus?.reason || "Move GalaxyHire to a writable Applications folder before updating.";
    if (state === "installing") return "Download complete. Applying the signed update in the background.";
    if (state === "downloading") return "Downloading the signed update. You can keep using GalaxyHire while this runs.";
  ...
  const install = async () => {
    if (installStatus && !installStatus.canUpdate) {
      setError(
        RELEASES_URL
          ? `${installStatus.reason} Download the latest build from ${RELEASES_URL} if you need to repair this install now.`
          // No release channel is configured, so pointing anywhere would be a guess.
          : `${installStatus.reason} Reinstall from your original source to repair this install.`,
      );
      setState("error");
      return;
    }
  ...
  return (
    <aside className="update-toast" role="status" aria-live="polite">
      <div>
        ...
        <strong>GalaxyHire {update.version}</strong>
        <p>{updateMessage}</p>
        {isBusy && (
        ...
        {state === "error" && <p className="update-error">{error || (RELEASES_URL ? `Update failed. Try again from ${RELEASES_URL}.` : "Update failed.")}</p>}
      </div>
