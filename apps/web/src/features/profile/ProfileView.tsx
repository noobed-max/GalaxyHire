...
import { applyProfileDeleteMarkers, entryTitle, normalizeProfileResponse, profileDeleteKey, profileDeletePath, profileHasDeleteMarker, removeProfileItem, type ProfileDeleteMarker } from "./profileUtils";
...
export function ProfileView({ api, setView }: { api: ApiFetch; setView: (v: View) => void }) {
  ...
  const fetchProfile = useCallback(async (options?: { errorPrefix?: string; suppressError?: boolean }) => {
    try {
      ...
      setProfile(applyLocalDeletes(normalizeProfileResponse(data), true));
      setProfileErr(null);
      return true;
    } catch (err: any) {
  ...
  useEffect(() => {
    const exportProfile = async () => {
      ...
      if (isTauri()) {
        try {
          // openGeneratedDocument, not openExternalUrl: this is our own blob: URL, and the
          // http(s)-only guard would refuse it.
          await openGeneratedDocument(url);
        } finally {
