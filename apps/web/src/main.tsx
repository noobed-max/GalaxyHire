  ...
  root.innerHTML = `
    <div style="min-height:100vh;display:grid;place-items:center;background:#f6f3ec;color:#201b16;padding:24px;font-family:Inter,Segoe UI,Arial,sans-serif">
      <section style="width:min(720px,100%);background:#fffdf8;border:1px solid #ded8cc;border-radius:12px;padding:28px;box-shadow:0 18px 48px rgba(32,27,22,.08)">
        <div style="font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:#7b7165">GalaxyHire startup error</div>
        <h1 style="font-size:28px;line-height:1.15;margin:10px 0 12px">The app shell could not finish loading</h1>
        <p style="color:#5d544b;line-height:1.6;margin:0 0 16px">Restart GalaxyHire once. If this remains visible, send this message with your OS and installer type.</p>
...
try {
  ...
  // Boot succeeded, so a future chunk failure may safely trigger the
  // ErrorBoundary's one-shot stale-tab reload again.
  sessionStorage.removeItem("jhm-chunk-reload");

  ReactDOM.createRoot(root).render(
    <React.StrictMode>
      {/* Last-resort boundary: the try/catch below only covers the synchronous
          initial render, not crashes on later re-renders. */}
      <ErrorBoundary label="GalaxyHire">
        <App />
      </ErrorBoundary>
    </React.StrictMode>,
  );
