...
const uninstallRegistryKeySuffix = "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GalaxyHire";
const vendorRegistryKeySuffix = "Software\\noobedmax\\GalaxyHire";
...
function windowsShortcutPaths() {
  if (process.platform !== "win32") return [];
  return [
    join(process.env.APPDATA || "", "Microsoft", "Windows", "Start Menu", "Programs", "GalaxyHire.lnk"),
    join(process.env.USERPROFILE || "", "Desktop", "GalaxyHire.lnk"),
    join(process.env.APPDATA || "", "Microsoft", "Internet Explorer", "Quick Launch", "User Pinned", "TaskBar", "GalaxyHire.lnk"),
  ].filter(Boolean);
}
...
async function cleanupInstalledPackage(installDir, registrySnapshots) {
  ...
  if (existsSync(uninstaller)) {
    ...
    if (result.status !== 0) {
      console.warn(`Cleanup warning: GalaxyHire uninstaller exited with ${result.status}.`);
    }
    await sleep(2500);
  }
