...
def _embeddings_actually_work() -> bool:
    """Whether semantic matching functions right now, regardless of the upstream runtime pack.

    This is the question the UI should have been asking all along. Dependencies here are installed
    by uv and the ONNX model is fetched on first run, so the pack JustHireMe used to ship its native
    libraries in is simply not part of how this project is installed.
    """
    from data.vector.embeddings import embedding_status

    try:
        status = embedding_status()
    except Exception:  # noqa: BLE001 - a diagnostic must never take down the status route
        return False
    # The hashing fallback is a real degradation, so it does not count as working.
    return status.get("status") == "ok" and "hashing embedder" not in str(status.get("model", ""))
...
def _runtime_payload(sync: dict | None = None) -> dict:
    ...
    # Report the capability, not legacy runtime-pack packaging.
    #
    # `runtime.ready` only ever meant "the upstream runtime-pack zip is unpacked on disk", which on
    # this project is never true. So the UI showed a permanent "Runtime pack required for semantic
    # matching / Install" banner while ONNX embeddings were loaded and working in the same process
    # — the startup log says `onnx active=True`. A prompt that is wrong every time teaches people to
    # ignore prompts.
    #
    # The legacy installer also downloaded and extracted an unsigned archive at runtime. That is
    # not a dependency path worth keeping for a capability that already works without it.
    working = _embeddings_actually_work()
    ready = working and not restart_required
    payload = {
        "ready": ready,
        "required": not working and not restart_required,
...
async def install_vector_runtime_endpoint():
    """Deliberately disabled. Kept as a route so an old client gets an explanation, not a 404.

    This used to download and extract an **unsigned zip at runtime** into the user's home directory,
    triggered by a banner that was displayed unconditionally because its readiness check could
    never pass here.

    Fetching and unpacking arbitrary archives from a repo this project does not control is not a
    dependency mechanism worth keeping — especially for a capability that already works: embeddings
    come from ONNX, installed through uv like everything else. If the native libraries are ever
    genuinely missing, the fix is a declared dependency, not a runtime download.
    """
    return _runtime_payload() | {
        "install_error": (
            "Runtime-pack installation is disabled. It fetched an unsigned archive from the "
            "legacy release location. Embeddings are "
            "provided by ONNX via the normal dependency install and need no separate pack."
        ),
    }
