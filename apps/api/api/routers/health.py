...
def _check_vector(repo: Repository) -> dict:
    try:
        ...
        if module is None:
            # The runtime-pack check below describes the Tauri desktop shell's bundled
            # libraries. Under uv/dev the package lives in the project venv, so give the
            # real import a chance before reporting the subsystem missing — importing is
            # what the rest of the app does, and it succeeds whenever lancedb is installed.
            try:
                import data.vector.connection as loaded  # noqa: PLC0415
