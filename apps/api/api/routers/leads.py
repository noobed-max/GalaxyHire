...
from core.types import FeedbackBody, FollowupBody, ManualLeadBody, StatusBody, StrictBody
from data.repository import Repository

# NOTE: must live at module level. Under `from __future__ import annotations`,
# a request-body model defined inside create_router() becomes an unresolvable
# ForwardRef for FastAPI, which then mis-reads the parameter as a *query* param
# (422 "missing query body") and breaks openapi.json generation.
class SelectDocumentBody(StrictBody):
    resume_document_id: str = Field(default="", max_length=32)
    cover_letter_document_id: str = Field(default="", max_length=32)
...
async def _run_relearn(ranking_service, manager) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            'suppressed exception in _run_relearn: %s', exc
        )
    finally:
...
def versioned_assets(job_id: str, base_dir: str) -> list[dict]:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in versioned_assets: %s', log_exc)
        return []
    for name in names:
...
def create_router(manager) -> APIRouter:
    ...
    async def cleanup_leads(repo: Repository = Depends(get_repository)):
        """Discard obvious junk rows (HN comments, non-job content)."""
        return await asyncio.to_thread(repo.leads.cleanup_bad_leads)
    ...
    @router.post("/leads/{job_id}/select-document")
    async def select_document(job_id: str, body: SelectDocumentBody, repo: Repository = Depends(get_repository)):
        """Attach an already-uploaded document (by tag) to this lead instead of
        generating. Only the file reference moves onto the lead; the document
        itself stays in the library for other applications."""
        job_id = _safe_job_id(job_id)
        lead = await asyncio.to_thread(repo.leads.get_lead_by_id, job_id)
        if not lead:
            raise HTTPException(status_code=404, detail="lead not found")
        ...
        if body.resume_document_id:
            doc = await asyncio.to_thread(repo.documents.get_document, body.resume_document_id)
            if not doc or doc["kind"] != "resume":
                raise HTTPException(status_code=404, detail="resume document not found")
            updates["resume"] = doc["file_path"]
        if body.cover_letter_document_id:
            doc = await asyncio.to_thread(repo.documents.get_document, body.cover_letter_document_id)
            if not doc or doc["kind"] != "cover_letter":
                raise HTTPException(status_code=404, detail="cover letter document not found")
            updates["cover_letter"] = doc["file_path"]
        if not updates:
            raise HTTPException(status_code=422, detail="nothing selected")
        await asyncio.to_thread(repo.leads.save_asset_package, job_id,
                                updates.get("resume", lead.get("asset_path", "")),
                                updates.get("cover_letter", lead.get("cover_letter_path", "")),
                                lead.get("selected_projects", []) or [],
                                None)
        fresh = await asyncio.to_thread(repo.leads.get_lead_by_id, job_id)
        await manager.broadcast({"type": "LEAD_UPDATED", "data": fresh})
    ...
    async def create_manual_lead(
        ...
        try:
            ...
            logging.getLogger(__name__).warning('suppressed exception in create_manual_lead: %s', exc)
