...
from api.scheduler import create_email_tick, create_ghost_tick, create_lifespan, create_scheduler
...
def _reserve_socket(preferred: int = 0) -> socket.socket:
    ...
    if sys.platform != "win32" and preferred:
...
def build_gateway_app():
    ...
    email_tick = create_email_tick(cm)
    lifespan = create_lifespan(_sched, ghost_tick, _log, email_tick)
    return create_app(
        lifespan=lifespan,
