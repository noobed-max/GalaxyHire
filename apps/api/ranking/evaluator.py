...
def _compact_json(value, limit: int = 11000) -> str:
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _compact_json: %s', log_exc)
...
def _evaluator_llm_requested(settings: dict | None = None) -> bool:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _evaluator_llm_requested: %s', log_exc)
        return False
...
def _normalize_llm_result(raw, baseline: dict) -> dict:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _normalize_llm_result: %s', log_exc)
        score = int(baseline.get("score") or 0)
