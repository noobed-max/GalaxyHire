...
def _embed_jd(text: str) -> list[float] | None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _embed_jd: %s', log_exc)
        return None
    try:
...
def _embedding_mode() -> str:
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _embedding_mode: %s', log_exc)
        return ""
...
def _vec_store():
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _vec_store: %s', log_exc)
        return None
...
def _available_tables(store) -> set[str]:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _available_tables: %s', log_exc)
        return set()
...
def _table_search(
    ...
    *,
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _table_search: %s', log_exc)
        return []
    try:
...
def _hash_embedding(text: str) -> list[float] | None:
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _hash_embedding: %s', log_exc)
        return None
