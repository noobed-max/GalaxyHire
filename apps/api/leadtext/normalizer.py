...
from leadtext.lead_intel import budget_from_text as budget_from_text
from leadtext.lead_intel import fit_bullets as fit_bullets
from leadtext.lead_intel import followup_sequence as followup_sequence
from leadtext.lead_intel import lead_id as lead_id
from leadtext.lead_intel import location_from_text as location_from_text
from leadtext.lead_intel import outreach_drafts as outreach_drafts
from leadtext.lead_intel import proof_snippet as proof_snippet
from leadtext.lead_intel import signal_quality as signal_quality
from leadtext.lead_intel import tech_stack_from_text as tech_stack_from_text
from leadtext.lead_intel import urgency_from_text as urgency_from_text
...
# One precompiled alternation per term group. The previous implementation
# re-escaped and re-searched a fresh pattern PER TERM PER LEAD, which made
# /api/v1/leads spend ~3s on a few hundred rows; these are built once and
# scan the text in a single pass each.
def _compile_terms(terms: tuple[str, ...]) -> re.Pattern[str]:
    escaped = "|".join(
        re.escape(t.strip()).replace(r"\ ", r"\s+") for t in terms if t.strip()
    )
    return re.compile(rf"(?<![a-z0-9])(?:{escaped})(?![a-z0-9])", flags=re.I)


_FRESHER_RE = _compile_terms(FRESHER_TERMS)
_JUNIOR_RE = _compile_terms(JUNIOR_TERMS)
_MID_RE = _compile_terms(MID_TERMS)
_SENIOR_RE = _compile_terms(SENIOR_TERMS)
...
def _has_seniority_term(text: str, terms: tuple[str, ...]) -> bool:
    pattern = {
        id(FRESHER_TERMS): _FRESHER_RE,
        id(JUNIOR_TERMS): _JUNIOR_RE,
        id(MID_TERMS): _MID_RE,
        id(SENIOR_TERMS): _SENIOR_RE,
    }.get(id(terms))
    if pattern is not None:
        return bool(pattern.search(text))
    return _compile_terms(terms).search(text) is not None


@lru_cache(maxsize=4096)
def _classify_text_cached(text: str) -> str:
...
def classify_job_seniority(lead: dict) -> str:
    # Seniority is a pure function of the lead's text; memoizing on that text
    # turns the per-request annotate pass over every stored lead into dict
    # lookups after the first occurrence of each posting.
    return _classify_text_cached(_lead_text(lead).lower())
