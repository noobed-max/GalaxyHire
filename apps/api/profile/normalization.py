...
def normalize_candidate_model(profile: CandidateProfile) -> CandidateProfile:
    from profile.ingest_parse import canonical_experience_key, canonical_project_key, split_points
    ...
    payload = normalize_profile_payload({
        ...
        "experience": [
            {"role": exp.role, "company": exp.co, "period": exp.period,
             "description": "\n".join(exp.points) if exp.points else exp.d, "skills": exp.s}
            for exp in profile.exp
        ],
        "projects": [
            {"title": project.title, "stack": project.stack, "repo": project.repo or "",
             "impact": "\n".join(project.points) if project.points else project.impact}
    ...
    exp_map = {
        canonical_experience_key(e.role, e.co): e
        for e in profile.exp
    }
    proj_map = {
        canonical_project_key(p.title, p.repo or ""): p
        for p in profile.projects
    }
    def exp_source(item):
        return exp_map.get(canonical_experience_key(item.get("role", ""), item.get("company", "")))

    def project_source(item):
        return proj_map.get(canonical_project_key(item["title"], item.get("repo") or ""))

    return CandidateProfile(
        n=clean_name or "Candidate",
        s=payload["candidate"].get("summary", "") or profile.s,
        resume_id=profile.resume_id,
        skills=[SkillEntry(n=item["name"], cat=item.get("category", "general")) for item in payload["skills"]],
        exp=[
            ExperienceEntry(
                role=item.get("role", ""),
                co=item.get("company", ""),
                ...
                matched_entity_id=(exp_source(item).matched_entity_id if exp_source(item) else ""),
                points=(list(exp_source(item).points) if exp_source(item) and exp_source(item).points else split_points(item.get("description", ""))),
                s=list(item.get("skills") or []),
                exact_matches=(exp_source(item).exact_matches if exp_source(item) else []),
                similar_pairs=(exp_source(item).similar_pairs if exp_source(item) else []),
                new_points=(exp_source(item).new_points if exp_source(item) else []),
            )
        ...
        projects=[
            ProjectEntry(
                ...
                matched_entity_id=(project_source(item).matched_entity_id if project_source(item) else ""),
                points=(list(project_source(item).points) if project_source(item) and project_source(item).points else split_points(item.get("impact", ""))),
                s=_stack_list(item.get("stack")),
                exact_matches=(project_source(item).exact_matches if project_source(item) else []),
                similar_pairs=(project_source(item).similar_pairs if project_source(item) else []),
                new_points=(project_source(item).new_points if project_source(item) else []),
            )
...
def normalize_experiences(raw_items: list[Any]) -> list[dict[str, Any]]:
    ...
    """
    from profile.ingest_parse import canonical_experience_key
    ...
    for raw in raw_items:
        ...
        if not description:
            highlights = item.get("highlights") or item.get("points") or item.get("bullets")
        ...
        key = canonical_experience_key(role, co)
        if not key:
            continue
        existing = index.get(key)
        if existing is not None:
            from profile.ingest_parse import split_points, canonical_point_key
            existing_bullets = split_points(existing.get("description", ""))
            incoming_bullets = split_points(description)
            seen_bullet_keys = {canonical_point_key(b) for b in existing_bullets}
            for b in incoming_bullets:
                bk = canonical_point_key(b)
                if bk and bk not in seen_bullet_keys:
                    existing_bullets.append(b)
                    seen_bullet_keys.add(bk)
...
def normalize_projects(raw_items: list[Any], *, known_skills: list[str] | None = None) -> list[dict[str, Any]]:
    from profile.ingest_parse import canonical_project_key
    ...
    for raw in raw_items:
        ...
        ident = canonical_project_key(title, repo)
        cleaned = {
            **item,
        ...
        if ident in seen:
            ...
            from profile.ingest_parse import split_points, canonical_point_key
            existing_impacts = split_points(existing.get("impact", ""))
            incoming_impacts = split_points(impact)
            seen_impact_keys = {canonical_point_key(b) for b in existing_impacts}
            for b in incoming_impacts:
                bk = canonical_point_key(b)
                if bk and bk not in seen_impact_keys:
                    existing_impacts.append(b)
                    seen_impact_keys.add(bk)
            existing["impact"] = "\n".join(existing_impacts)
            continue
        seen[ident] = cleaned
        out.append(cleaned)
...
def _clean_project_detail(value: str) -> str:
    # Keep explicit semantic bullet boundaries.  Collapsing all whitespace
    # here used to turn every project bullet into one opaque paragraph before
    # point extraction.
    parts: list[str] = []
...
def _repo_title_from_url(url: str) -> str:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _repo_title_from_url: %s', log_exc)
        return ""
...
def normalize_bullet_point(text: str) -> str:
    """Deterministic normalized alphanumeric key for exact point matching."""
    from profile.ingest_parse import canonical_point_key
    return canonical_point_key(text)
