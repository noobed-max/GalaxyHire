...
Writes the parsed profile (CandidateProfile) into the Kuzu graph (candidate/skill/project/
    ...
    try:
        ...
        if "duplicated primary key" in str(exc).lower() and len(props) > 1:
            try:
                ...
                logging.getLogger(__name__).warning('suppressed exception in _put_node: %s', log_exc)
                return
        logging.getLogger(__name__).warning('suppressed exception in _put_node: %s', exc)
...
def _put_rel(a: str, aid: str, b: str, bid: str, rel: str):
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _put_rel: %s', log_exc)
        pass
    ...
    if name in vec_table_names():
        ...
        if ids:
            ...
            try:
                ...
                logging.getLogger(__name__).warning('suppressed exception in _put_vec: %s', log_exc)
                pass
        table.add(rows)
    else:
...
def _vectors(p: CandidateProfile):
    try:
