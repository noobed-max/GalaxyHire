...
def safe_graph_step(fn, label: str, errors: list[str], default=None):
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in safe_graph_step: %s', exc)
...
def sync_vectors_from_graph() -> dict:
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in sync_vectors_from_graph: %s', exc)
...
def embedding_space(repo: Repository, limit: int = 80) -> dict:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in embedding_space: %s', exc)
    ...
    for table_name in tables:
        try:
            ...
            logging.getLogger(__name__).warning('suppressed exception in embedding_space: %s', log_exc)
            rows = []
        for row in rows:
...
def project_vector(vector: list) -> tuple[float, float, float]:
    ...
    for idx, raw in enumerate(vector):
        try:
            ...
            logging.getLogger(__name__).warning('suppressed exception in project_vector: %s', log_exc)
            continue
        if value == 0:
            continue
...
def _normalize_table_names(raw) -> list[str]:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in vector_table_names: %s', log_exc)
        pass
    try:
