...
def record_exception(exc: BaseException, *, domain: str = "api", request_id: str = "", path: str = "") -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).debug('suppressed exception in record_exception: %s', log_exc)
        return
...
def _rotate_error_log(max_lines: int = 500) -> None:
    try:
        ...
        logging.getLogger(__name__).debug('suppressed exception in _rotate_error_log: %s', log_exc)
        return
...
def log_error(exc: BaseException | str, context: dict | None = None) -> None:
    try:
        ...
        logging.getLogger(__name__).debug('suppressed exception in log_error: %s', log_exc)
        return
...
def record_error(error_type: str, message: str = "", source: str = "") -> None:
    try:
        ...
        logging.getLogger(__name__).debug('suppressed exception in record_error: %s', log_exc)
        return
...
def get_top_errors(limit: int = 10, days: int = 7) -> list[dict]:
    try:
        ...
        logging.getLogger(__name__).debug('suppressed exception in get_top_errors: %s', log_exc)
        return []
...
def get_error_count(hours: int = 24) -> int:
    try:
        ...
        logging.getLogger(__name__).debug('suppressed exception in get_error_count: %s', log_exc)
        return 0
