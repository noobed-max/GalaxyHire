...
class _DocPackage(BaseModel):
    ...
    verified_jd_keywords: list[str] = Field(
        default_factory=list,
        description=(
            "The supplied ATS keyword candidates that are genuine screening requirements for this "
            "specific job. Every value must come from the supplied candidate list."
        ),
    )
    rejected_jd_keywords: list[str] = Field(
        default_factory=list,
        description=(
            "Supplied keyword candidates that are mentioned but are not meaningful requirements "
            "for this job. Every value must come from the supplied candidate list."
        ),
    )
