...
def test_resume_fallback_prioritizes_jd_keywords_and_evidence():
    ...
    # No SUMMARY section (My-Resume-Maker format): contact line, then skills.
    assert "## SUMMARY" not in resume
...
def test_resume_fallback_scrubs_contact_summary_and_project_noise():
    ...
    # Contact-scrub assertions: the free-text junk in profile `s` (fake phone, second
    # github link, "Targeting" line) must not leak anywhere. The REAL identity email may
    # legitimately sit on the contact line. There is no SUMMARY section anymore.
    assert "## SUMMARY" not in resume
    assert "+1 555 010 0001" not in resume
    assert "SOMA" not in resume
    assert "Targeting" not in resume
    assert "wellfound.com/jobs" not in resume
