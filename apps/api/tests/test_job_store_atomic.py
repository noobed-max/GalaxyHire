...
def test_job_store_atomic_update_and_indexes(tmp_path):
    ...
    script = (
        ...
        "c = store.create('scan'); store.update(c.job_id, status='cancel_requested'); store.update(c.job_id, progress=42)\n"
        "assert store.get(c.job_id).status == 'cancel_requested' and store.get(c.job_id).progress == 42\n"
