...
# The Tauri shell lives with the frontend in GalaxyHire (ARCHITECTURE.md D3), not beside the API.
ROOT = BACKEND.parents[1] / "web"

# These assert on JustHireMe's packaged Windows release: installer slimness, the OTA runtime
# pack, and the frozen-backend spec. GalaxyHire is browser-first and has no packaging pipeline
# yet, and src-tauri/resources was excluded from the import (132 MB of bundled runtime). The
# checks are kept rather than deleted so re-adding desktop packaging re-enables them.
pytestmark = pytest.mark.skipif(
    not (ROOT / "src-tauri" / "resources").exists(),
    reason="no desktop packaging pipeline yet; re-enables once src-tauri/resources exists",
)
