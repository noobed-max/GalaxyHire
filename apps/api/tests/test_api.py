...
_TEST_DATA_DIR = Path(tempfile.mkdtemp(prefix="galaxyhire-api-tests-"))
os.environ["LOCALAPPDATA"] = str(_TEST_DATA_DIR)
os.environ["JHM_APP_DATA_DIR"] = str(_TEST_DATA_DIR)
...
class TestIngestionEndpoints(unittest.TestCase):
    def test_resume_ingest_accepts_text_upload_and_returns_profile(self):
        ...
        from models.schema import CandidateProfile, SkillEntry

        class FakeProfileService:
            async def ingest_resume(self, raw="", pdf_path=None):
                self.raw = raw
                self.pdf_path = pdf_path
                return CandidateProfile(n="Jane Doe", s="Applied AI engineer", skills=[SkillEntry(n="Python", cat="technical")])
