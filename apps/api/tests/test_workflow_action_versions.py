...
# GalaxyHire's repo root is two levels above the app (apps/api/tests -> apps -> repo).
ROOT = Path(__file__).resolve().parents[3]
WORKFLOWS = [ROOT / ".github/workflows/ci.yml", ROOT / ".github/workflows/release.yml"]

# GalaxyHire has not defined its own CI workflows yet — JustHireMe's were deliberately not
# carried over, since they build its Tauri release rather than this stack. The assertions stay
# so they start enforcing action pins the moment workflows appear.
pytestmark = pytest.mark.skipif(
    not any(w.exists() for w in WORKFLOWS),
    reason="no .github/workflows yet; re-enables automatically once CI is defined",
)
