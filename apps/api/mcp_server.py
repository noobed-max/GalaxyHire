...
from leadtext.lead_intel import (
    budget_from_text,
    company_from_text,
    location_from_text,
    signal_quality,
...
from leadtext.quality_gate import evaluate_lead_quality
...
def _handle(request: Json) -> Json | None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _handle: %s', exc)
        return {
            "jsonrpc": "2.0",
            "id": req_id,
...
def main() -> None:
    for line in sys.stdin:
        ...
        try:
            ...
            logging.getLogger(__name__).warning('suppressed exception in main: %s', exc)
            response = {
                "jsonrpc": "2.0",
                "id": None,
