...
def _vector_rows(table_name: str, limit: int = 500) -> list[dict]:
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _vector_rows: %s', log_exc)
        return []
...
def delete_vec_rows(table_name: str, ids: list[str]) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in delete_vec_rows: %s', log_exc)
        pass
...
def drop_vec_table(table_name: str) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in drop_vec_table: %s', log_exc)
...
def current_embedding_dim() -> int | None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in current_embedding_dim: %s', log_exc)
    return None
...
def prune_bad_vector_rows() -> int:
    ...
    for table_name in ["profile", "candidates", "skills", "projects", "experiences", "credentials"]:
        try:
            ...
            logging.getLogger(__name__).warning('suppressed exception in prune_bad_vector_rows: %s', log_exc)
            continue
    return deleted
...
def _normalize_table_names(raw) -> list[str]:
    ...
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in vec_table_names: %s', log_exc)
    try:
...
def _rows_for_existing_table(table_name: str, rows: list[dict]) -> list[dict]:
    try:
        ...
        logging.getLogger(__name__).warning('suppressed exception in _rows_for_existing_table: %s', log_exc)
        return rows
