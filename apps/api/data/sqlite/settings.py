...
_LLM_PROVIDER_VALUES = [
    ...
    "sambanova", "qwen", "azure", "opencode", "custom",
...
# Per-step provider values go through the same allowlist as the global provider.
# Without explicit schema entries SettingsBody's flexible extra fields bypassed
# validation for these routes.
for _step in ("scout", "generator", "ingestor", "actuator"):
    SETTINGS_SCHEMA[f"{_step}_provider"] = {
        "type": "str",
        "allowed": _LLM_PROVIDER_VALUES,
        "default": "",
    }
...
def delete_setting(key: str, db_path: str = DEFAULT_DB_PATH) -> bool:
    """Delete a setting key completely from the settings table."""
    _ensure_settings_table(db_path)
    conn = get_connection(db_path)
    try:
