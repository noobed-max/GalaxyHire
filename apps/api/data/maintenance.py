...
`reset_all_data` clears the app-local CRM/leads, profile knowledge graph, vector tables, and
generated documents—leaving schemas intact. The API route additionally clears user-authored corpus
scrape history while retaining canonical jobs, since the corpus is reusable product data rather
than a user profile store.
...
def _backup_sqlite_before_wipe(summary: dict) -> None:
    """Snapshot crm.db (plus WAL/SHM if present) next to itself before the wipe.

    The Danger-zone reset is irreversible by design, but it also once took a
    718-lead pipeline with it. A pre-wipe copy turns any accidental click into
    a recoverable mistake; keep exactly one (last) backup to bound disk use."""
    import shutil
    ...
    if not os.path.exists(db_path):
        return
    try:
        backup = db_path + ".pre-reset.bak"
        for suffix in ("", "-wal", "-shm"):
            src = db_path + suffix
            if os.path.exists(src):
                shutil.copy2(src, backup + suffix)
        _log.info("pre-reset sqlite backup written to %s", backup)
    except Exception as exc:
        summary["errors"].append(f"pre-reset backup: {exc}")
...
def reset_all_data(*, clear_settings: bool = False) -> dict:
    ...
    _backup_sqlite_before_wipe(summary)
